...
> 💡 **Codex 포트폴리오 작업용 가이드**:
> 본 문서는 코덱스(Codex)가 포트폴리오 웹사이트, 노션 기술 블로그, 이력서 프로젝트 설명, PPT 슬라이드 등을 손쉽게 제작할 수 있도록 실제 소스코드와 장애 복구 과정, 정량적 개선 지표를 집대성한 공식 케이스 스터디 작업집입니다.
> 원본 상세 문서는 `docs/PORTFOLIO_CASE_STUDY_SUPABASE_OUTAGE.md`에도 영구 보존되어 있습니다.
...
| 항목 | 상세 내용 |
| :--- | :--- |
| **프로젝트명** | 드래곤 빌리지 3 카드교환소 (실서비스 라이브 운영) |
...
| **핵심 성과** | 17개 글로벌 리전 스캔 스크립트로 활성 풀러 발굴 ➡️ 4,406건 데이터 6초 만에 무결성 이전 복구 ➡️ 웹소켓 전환 및 하이브리드 캐싱 아키텍처로 Egress 트래픽 99% 영구 절감 달성 |
...
> 
> 저는 즉시 두 단계로 대응했습니다. 첫째, 긴급 복구를 위해 전 세계 17개 AWS 리전의 풀러 주소를 자동 스캔하는 도구를 직접 작성해 접속 통로를 확보한 뒤, `pg.Pool` 기반 벌크 INSERT로 4,406건의 데이터를 6초 만에 신규 DB로 100% 무결성 이전했습니다.
