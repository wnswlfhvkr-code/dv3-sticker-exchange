...
function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // 포커스가 input 또는 textarea에 가 있다면 작동 방지
      ...
      if (e.key === '4') {
        e.preventDefault();
        // 수정 모달이 열려있는 상태라면 수정 모달 카테고리 전환
        if (isEditModalOpen) {
  ...
  const toggleStickerSelection = (stickerId, targetMode) => {
    ...
    if (myPost) {
      updatePostStickersDirectly(myPost.id, nextHaves, nextWants);
    }
  };
            ...
            <div className="glass-card" style={{ width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '1.2rem', padding: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', width: '100%', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '0.75rem', flexWrap: 'nowrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    ...
                    >
                      <ChevronLeft size={16} />
                      <span style={{ fontSize: '0.6rem', opacity: 0.65, marginLeft: '2px' }}>(4)</span>
                    </button>
                    ...
                    <button
                      ...
                      title="다음 카테고리 (단축키: 6)"
                    >
                      <span style={{ fontSize: '0.6rem', opacity: 0.65, marginRight: '2px' }}>(6)</span>
              ...
              <div style={{ 
                ...
                {Array.from({ length: 9 }).map((_, slotIdx) => {
                  ...
                  let borderStyle = isGolden ? '2px solid rgba(156, 163, 175, 0.3)' : '2px solid var(--border-color)';
                  let backgroundStyle = isGolden ? 'rgba(0, 0, 0, 0.4)' : '#1d2025';
                  let shadowStyle = 'none';
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        ...
                        boxShadow: shadowStyle,
                        opacity: isGolden ? 0.3 : (isSelected ? 1 : 0.65),
                        filter: isGolden ? 'grayscale(100%) contrast(70%)' : (isSelected ? 'none' : 'grayscale(60%) brightness(70%)'),
                        paddingTop: '0.5rem'
                      }}
                      className="slot-item"
                    >
        ...
        <div style={{
          ...
          <div className="glass-card" style={{
            ...
            <form onSubmit={handleUpdatePost} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {/* [수정 모달 전용] 추가할 카드 선택 셀렉터 (3x3 격자도감화) */}
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', background: 'rgba(255,255,255,0.02)', padding: '0.8rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '2px' }}>
                      💡 좌클릭: 줄 수 있는 카드 담기 / 우클릭: 받고 싶은 카드 담기
                    </div>
                  </div>
                );
              })()}
