          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {boardVM.posts.length === 0 ? (
              ...
              boardVM.posts.map(post => (
                <article key={post.id} className="glass-card" style={{ padding: '1rem', borderRadius: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 0 }}>
                      <h3 style={{ margin: '0 0 0.35rem', fontSize: '1rem', color: '#fff', wordBreak: 'break-word' }}>{post.title}</h3>
