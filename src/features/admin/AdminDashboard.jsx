            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {statsLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.4rem' }}>
                  {statsData.isDbFailed && (
                    <div style={{ background: 'rgba(239, 68, 68, 0.08)', border: '1px dashed rgba(239, 68, 68, 0.3)', borderRadius: '12px', padding: '0.85rem 1rem', fontSize: '0.8rem', color: '#fca5a5', lineHeight: '1.5', textAlign: 'left' }}>
                      <strong style={{ display: 'block', fontSize: '0.85rem', color: '#f87171', marginBottom: '4px' }}>⚠️ 실서버 접속 통계 테이블(visit_logs) 감지 실패</strong>
                      익명 접속자의 DB 쓰기 권한이 없거나 테이블이 누락되어 임시 로컬 모드로 작동 중입니다. <br/>
                      이 상태에서는 다른 사람들의 실제 모바일/PC 접속이 통합 집계되지 않습니다. 실시간 집계를 활성화하려면 <strong>Supabase SQL Editor</strong>에서 아래 쿼리를 실행해 주세요:
                      <pre style={{ background: 'rgba(0,0,0,0.3)', padding: '0.5rem', borderRadius: '6px', fontSize: '0.72rem', marginTop: '6px', overflowX: 'auto', border: '1px solid rgba(255,255,255,0.05)', color: '#d1d5db', userSelect: 'all' }}>
